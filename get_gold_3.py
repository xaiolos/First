...
def getPageContent(self):
    myUrl='https://cjb.alipay.com/gold/guide.htm;jsessionid=72ADD6AFEED20C654D2FF2A2D0E11CA9'+str(self.page)
  ...
  #  unicodePage=myPgae.decode('utf-8')
    #根据正则表达式拿到所有的内容
    #  myItems=re.findall('<p.*?class="date">(.*?)<i',myPgae,re.S)
    myItems=re.findall('<span.*?class="ft-56">(.*?)</span>',myPgae,re.S)
    items=[]
